...
void Logger::DeleteThreadLabel()
{
    std::lock_guard<std::mutex> lock(_mutex);
    Logger::_threadsLabels.erase(std::this_thread::get_id());
}

void Logger::DeleteThreadLabel(std::thread::id threadID)
{
    Logger::_threadsLabels.erase(threadID);
}
