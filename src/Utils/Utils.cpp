...
sf::Vector2f Utils::fromAngle(float radAngle, float length) {
    return {
        length / cos(radAngle),
        length * sin(radAngle)
    };
...
double Utils::dist(const sf::Vector2f &a, const sf::Vector2f &b) {
    return Utils::mag(a - b);
}

double Utils::mag(const sf::Vector2f &v) {
    return sqrt(Utils::magSq(v));
}

double Utils::magSq(const sf::Vector2f &v) {
    return v.x * v.x + v.y * v.y;
}

double Utils::map(double n, double start1, double end1, float start2, float end2, bool withinBounds)
{
    double newval = (n - start1) / (end1 - start1) * (end2 - start2) + start2;

    if (!withinBounds)
        return newval;

    if (start2 < end2)
        return Utils::constrain(newval, start2, end2);
    else
        return Utils::constrain(newval, end2, start2);
}

double Utils::constrain(double n, double low, double high) {
    return std::max(std::min(n, high), low);
}

double Utils::heading(const sf::Vector2f &v) {
